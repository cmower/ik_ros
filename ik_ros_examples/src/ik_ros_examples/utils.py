...
class FigureEightNode:
    ...
    def main_loop(self, event):
        ...
        t = (self.tf.header.stamp - self.start_time).to_sec()*0.3
        self.tf.transform.translation.x = np.sin(t * 2.0 * np.pi * 0.5) * 0.05
        self.tf.transform.translation.y = np.sin(t * np.pi * 0.5) * 0.1
